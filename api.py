...
def upload_file():
    ...
    table_data_markdown = pd.DataFrame(table_data, columns=tuple(result.keys())).to_markdown()
    data = pd.DataFrame(table_data, columns=tuple(result.keys())).to_json(orient="records")
    ...
    Based on the Table Schema, Conversation History, SQL Query and SQL Response, ANSWER LIKE THE BELOW JSON STRUCTURE, MAKE THE KEY AS "Insights and Analysis" and provide the three Insights in List formay:
        ...
        <curlybraces start>
            Insights and Analysis:"Give atleast 5 Insights and Analysis about the SQL Response. Each of the insights will be in an individual Lists and Do not include escape characters"
        <curlybraces end>
    ...
    summary = summary_chain.invoke({"schema":schema,"question":request.json["query"],"query":query,"response":table_data_markdown})
    summary= summary.replace("\n", "").replace("\t", "").replace("\\", "")
    
    return {"query":query,"data":data,"summary":summary}
